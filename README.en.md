...
During connection setup, the server handles a code, device public keys, source IP addresses, SDP, and ICE candidates. This state is bounded and expires quickly. It is deleted once the peer connection is ready. See [Privacy](./docs/privacy.en.md) for the full boundary.
...
The default build contains public STUN URLs only. It contains no TURN endpoint or credential. Users can add a TURN server in Settings, or operators can follow the [coturn guide](./docs/turn.en.md). When a connection uses TURN, Peerto periodically renegotiates it and tries to upgrade to a direct path. Enable Relay only to keep a connection on TURN.
...
- [Docker and production deployment](./docs/deployment.en.md)
- [Deploying and using coturn](./docs/turn.en.md)
- [Cloudflare Tunnel, rate limits, and Turnstile](./docs/cloudflare.en.md)
- [Connection troubleshooting](./docs/troubleshooting.en.md)
- [Project structure](./ARCHITECTURE.en.md)
- [Requirements and protocol boundaries](./REQUIREMENTS.en.md)

## Contributing

Read [CONTRIBUTING.en.md](./CONTRIBUTING.en.md) before opening a pull request. Report vulnerabilities using [SECURITY.en.md](./SECURITY.en.md), not a public issue.
